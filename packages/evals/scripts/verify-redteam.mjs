...
// count designops/* errors (all five rules at error) and run tsc. An escape
...
for (const dir of dirs) {
  ...
  const byRule = {}
  const findings = []
  let designopsErrors = 0
  for (const r of results) {
    for (const m of r.messages) {
      if (!m.ruleId?.startsWith("designops/")) continue
      designopsErrors++
