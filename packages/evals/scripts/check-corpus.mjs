...
// DESIGNOPS_UI_DIR point at a local checkout instead.
...
const counts = {}
const fatal = []
for (const rule of Object.keys(plugin.rules)) counts[`designops/${rule}`] = 0
for (const result of results) {
  for (const msg of result.messages) {
    if (msg.fatal) fatal.push(`${result.filePath}:${msg.line}: ${msg.message}`)
    if (msg.ruleId?.startsWith("designops/")) counts[msg.ruleId]++
  }
}
if (!results.length) {
