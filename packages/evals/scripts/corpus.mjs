#!/usr/bin/env node
// Runs the @designops/lint rules across the registry (report-only) to
...
// DESIGNOPS_LINT_PLUGIN points at a built plugin file to measure a build
// that is not the installed one (a side build, a candidate release).
const { plugin } = await import(
  process.env.DESIGNOPS_LINT_PLUGIN
    ? pathToFileURL(path.resolve(process.env.DESIGNOPS_LINT_PLUGIN)).href
    : "@designops/lint"
)

// The registry is fetched at the commit pinned in scripts/registry.json.
// --dir or DESIGNOPS_UI_DIR point at a local checkout instead.
...
const targetDir =
  dirIdx !== -1
    ...
    : process.env.DESIGNOPS_UI_DIR
      ? path.join(process.env.DESIGNOPS_UI_DIR, "apps/v4/registry/new-york-v4")
      : ensureRegistry()
...
if (onlyRule) {
  const id = onlyRule.startsWith("designops/")
    ? onlyRule
    : `designops/${onlyRule}`
  console.log(`\nEvery finding of ${id}:`)
