...
- designops/no-restyle: className passed to a ui component may only adjust layout (margin, width, grid/flex placement, visibility). Appearance (colors, typography, spacing, shape, effects, motion) must come from the component's own variants. Use an existing variant if one matches the intent; add a new variant to the component's file in components/ui only if the design explicitly calls for a treatment none of the existing variants provides.
- designops/no-raw-colors: color utilities must use the theme's declared tokens (bg-primary, text-muted-foreground), never the raw Tailwind palette (bg-pink-500) or an undeclared name. To add a color, declare it in app/globals.css (a --color-<name> entry in the @theme block, backed by a :root variable), then use bg-<name>.
- designops/no-arbitrary-values: arbitrary values on appearance utilities (p-[13px], text-[11px], border-[#E4E4E7]) are banned on any element. Use theme tokens and scale values (p-3, text-xs, border-border); for sizes and spacing, use the nearest scale value.
- designops/no-inline-styles: no style attribute except CSS custom properties, and a custom property must not hardcode a color.
- designops/require-static-classes: className on ui components must be statically analyzable strings.`
...
The project's linter (@designops/lint) reports these violations:
...
This project enforces styling rules with a linter (@designops/lint). Review ${task.file} against the rules below and fix every violation you find.
