...
// work dir with every @designops/lint rule at error and returns findings.
...
// Lints the work dir. Findings are the designops rules' diagnostics plus
...
export async function lintWorkdir(workdir, { expectFiles = [] } = {}) {
  ...
  for (const result of results) {
    for (const msg of result.messages) {
      ...
      if (!msg.ruleId?.startsWith("designops/")) continue
      findings.push({
        file: path.relative(workdir, result.filePath),
        line: msg.line,
        rule: msg.ruleId,
