// Packs @designops/lint the way `npm publish` would, installs the tarball
...
const dir = fs.mkdtempSync(path.join(os.tmpdir(), "designops-lint-smoke-"))
...
try {
  ...
  write(
    "eslint.config.mjs",
    `import { plugin as designops } from "@designops/lint"\nimport parser from "@typescript-eslint/parser"\nexport default [\n  { files: ["**/*.tsx"], languageOptions: { parser, parserOptions: { ecmaFeatures: { jsx: true } } }, plugins: { designops }, rules: { "designops/no-restyle": ["error", { allow: ["layout"] }], "designops/no-raw-colors": "error", "designops/no-arbitrary-values": "error", "designops/no-inline-styles": "error", "designops/require-static-classes": "error", "designops/no-unknown-classes": "error" } },\n  { files: ["components/ui/**"], rules: { "designops/no-restyle": "off", "designops/require-static-classes": "off", "designops/no-arbitrary-values": "off" } },\n]\n`
  )
  write(
    ".oxlintrc.json",
    JSON.stringify({
      ...
      overrides: [
        {
          files: ["components/ui/**"],
          rules: {
            "designops/no-restyle": "off",
            "designops/no-arbitrary-values": "off",
          },
        },
      ],
    })
  ...
  const installed = JSON.parse(
    fs.readFileSync(
      path.join(dir, "node_modules/@designops/lint/package.json"),
      "utf-8"
    )
  )
  ...
  const probe = spawnSync(
    ...
    [
      "-e",
      `const { createRequire } = require("node:module"); const r = createRequire(require.resolve("@designops/lint", { paths: [process.cwd()] })); try { r("oxc-parser"); console.log("oxc") } catch (e) { console.log("none: " + e.message) }`,
    ],
    { cwd: dir, encoding: "utf-8" }
  )
  expect(
    `installed @designops/lint@${installed.version}; oxc-parser resolves from it`,
  ...
  expect(
    ...
    oout,
    /designops\(no-restyle\): "bg-pink-500" is not allowed on <Button>/
  )
