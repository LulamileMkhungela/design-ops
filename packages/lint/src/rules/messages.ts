...
function noteFor(context: any) {
  const note =
    context.settings?.designops?.note ?? context.settings?.shadcn?.note
  if (note == null) return ""
  if (typeof note !== "string") {
    warnOnce(
      "settings:note",
      "settings.designops.note must be a string; it is ignored."
    )
    return ""
  }
  return note.trim()
  ...
  for (const [, key] of message.matchAll(PLACEHOLDER)) {
    ...
    if (nearest) {
      warnOnce(
        ...
        `Unknown message placeholder "{{${key}}}" in ${label.startsWith("designops/") ? label : `contract "${label}"`}. Did you mean "{{${nearest}}}"? It will remain literal.`
      )
    }
  }
}
