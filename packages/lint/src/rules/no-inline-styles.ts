  ...
  const patterns = (entries ?? []).map((entry) => {
    ...
    if (!custom && !/^[a-zA-Z*][a-zA-Z*-]*$/.test(entry)) {
      throw new ContractConfigError(
        `designops/no-inline-styles: entry "${entry}" is not a CSS property name (backgroundColor, background-color, border-*, --chart-1), so it would match nothing.`
      )
    }
...
export const noInlineStyles = {
  meta: {
    ...
    docs: {
      ...
      url: "https://github.com/LulamileMkhungela/design-ops/blob/main/docs/rules/no-inline-styles.md",
    },
    schema: [
      {
        type: "object",
  ...
  create(context: any) {
    ...
    const emit = reporter(context, MESSAGES, {
      rule: "designops/no-inline-styles",
