// settings.designops: the recognition options written once instead of once
...
function strings(value: unknown, key: string) {
  ...
  if (!list.every((v) => typeof v === "string")) {
    warnOnce(
      `settings:${key}`,
      `settings.designops.${key} must be a string or an array of strings; it is ignored.`
    )
    return null
  }
...
export function withSettings<T extends Record<string, unknown>>(
  ...
  const settings = context.settings?.designops ?? context.settings?.shadcn
