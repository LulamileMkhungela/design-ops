...
export function classifierFor(fromFile?: string) {
  ...
  if (!classifier) {
    try {
      ...
      warnOnce(
        "classifier:fallback",
        `The installed cn's grammar could not be loaded (${(error as Error).message}); using the grammar bundled with @designops/lint. Update @designops/lint.`
      )
    ...
    if (unknown.length) {
      warnOnce(
        `groups:${unknown.join(",")}`,
        `The installed cn declares class groups this version of @designops/lint has no category for: ${unknown.join(", ")}. They are treated as layout. Update @designops/lint.`
      )
    }
  }
  return classifier
