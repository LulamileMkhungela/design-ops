...
function transportFailed(reason: string) {
  ...
  if (restarts++ >= 1) {
    ...
    warnOnce(
      "tailwind:off",
      `The Tailwind worker failed twice (${reason}); no-unknown-classes is using the grammar bundled with @designops/lint for the rest of this run.`
    )
  }
  return null
}
...
function start() {
  ...
  if (!file) {
    ...
    warnOnce(
      "tailwind:off",
      "The Tailwind worker script was not found next to @designops/lint; no-unknown-classes is using the bundled grammar."
    )
    return null
  }
  try {
...
function themeFailed(cssFile: string, reason: string) {
  ...
  warnOnce(
    ...
    `The Tailwind theme at ${cssFile} could not be built (${reason}); no-unknown-classes is using the grammar bundled with @designops/lint there until it can.`
  )
  return null
}
