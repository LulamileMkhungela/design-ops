  ...
  warnOnce(
    ...
    `components.json sets tailwind.css to ${relativeTo(project.root, file)}, which does not import Tailwind. ${
      discovered
        ...
        : "No stylesheet importing Tailwind was found under the project, so no-unknown-classes is using the grammar bundled with @designops/lint until the path is fixed."
    }`
  )
  return discovered
}
