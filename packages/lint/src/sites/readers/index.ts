  ...
  return {
    ...
    create(context: any) {
      ...
      if (!services?.isSvelte && isSfc(fileOf(context) ?? "")) {
        warnOnce(
          "templates:unread",
          "Templates in .svelte and .vue files are read under ESLint with svelte-eslint-parser or vue-eslint-parser. This run has no template parser, so only their script blocks are linted. See https://github.com/LulamileMkhungela/design-ops/blob/main/docs/vue.md and https://github.com/LulamileMkhungela/design-ops/blob/main/docs/svelte.md."
        )
      }
      return visitors
    },
