...
const RULE_NAMES = Object.keys(plugin.rules).map((name) => `designops/${name}`)
...
import { plugin as designops } from "../dist/index.js"

export default defineConfig([
  { plugins: { designops }, rules: { /*|*/ } },
])
`
...
export const config: Linter.Config = { plugins: { designops }, rules: { /*|*/ } }
`
...
describe("flat config completion", () => {
  test("the augmentation names every rule in plugin.rules and nothing else", () => {
    ...
    const declared = [...block.matchAll(/"(designops\/[a-z-]+)"\?:/g)].map(
      (m) => m[1]
    )
...
function declaredOptions() {
  ...
  const visit = (node: ts.Node) => {
    ...
    if (
      ...
      node.name.text.startsWith("designops/") &&
      node.type &&
...
describe("the augmentation's option names", () => {
  test.each(Object.entries(plugin.rules))(
    "designops/%s offers exactly the options its schema accepts",
    (name, rule) => {
      const declared = declaredOptions().get(`designops/${name}`)
