...
describe("Svelte templates", () => {
  ...
  it("names a component after its file, through a namespace import", () => {
    expect(found(`<Card.Title class="text-2xl">Hi</Card.Title>`)).toEqual([
      `designops/no-restyle: "text-2xl" is not allowed on <CardTitle>: <CardTitle> owns its typography`,
    ])
  ...
  it("lists the sizes a tv() in the module script declares", () => {
    ...
    expect(message.ruleId).toBe("designops/no-restyle")
  ...
  it("reads text and mustache parts as one template", () => {
    expect(
      ...
      `designops/no-restyle: "bg-primary" is not allowed on <Card>: <Card> owns its color`,
    ])
    // A glued interpolation is not a class anyone wrote.
    expect(found(`<Card.Root class="p-{tone}">x</Card.Root>`)).toEqual([
      "designops/require-static-classes: Dynamically built className on <Card> cannot be checked",
    ])
  })
  ...
  it("resolves an identifier in the template to the script", () => {
    expect(
      ...
      `designops/no-restyle: "text-lg" is not allowed on <CardContent>: <CardContent> owns its typography`,
    ])
  })
  ...
  it("reads arrays and objects the way clsx does", () => {
    expect(
      ...
      `designops/no-arbitrary-values: "text-[#123456]" hardcodes a color and no declared theme color is close to it`,
      `designops/no-raw-colors: "bg-red-500" uses the raw Tailwind palette`,
    ])
  })

  it("reads class: directives", () => {
    expect(found(`<Button class:bg-primary={active}>x</Button>`)).toEqual([
      `designops/no-restyle: "bg-primary" is not allowed on <Button>: <Button> owns its color`,
    ])
  ...
  it("reads style attributes and style: directives", () => {
    expect(
      ...
      "designops/no-inline-styles: Inline style sets color",
      "designops/no-inline-styles: Custom property --tone hardcodes a color",
      "designops/no-inline-styles: Inline style sets background-color",
      "designops/no-inline-styles: Custom property --chart hardcodes a color",
    ])
  })
  ...
  it("keeps a ; inside a quoted or parenthesized style value", () => {
    expect(
      ...
      "designops/no-inline-styles: Inline style sets background",
      "designops/no-inline-styles: Custom property --tone hardcodes a color",
    ])
  })
  ...
  it("knows the classes its own style block declares", () => {
    const rules = { "designops/no-unknown-classes": "error" }
    ...
    expect(
      summarize(
        lint("", `const chip = cn("bg-red-500");`).filter(
          (m) => m.ruleId === "designops/no-raw-colors"
        )
      )
    ).toHaveLength(1)
  })
