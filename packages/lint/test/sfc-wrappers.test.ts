...
describe("single-file component wrappers", () => {
  ...
  it("judges a Svelte wrapper by its target's contract", () => {
    expect(
      ...
      `designops/no-restyle: "rounded-none" is not allowed on <SaveButton>: <SaveButton> forwards className to <Button>, which owns its shape`,
    ])
  })
  ...
  it("judges a Vue wrapper by its target's contract", () => {
    expect(
      ...
      `designops/no-restyle: "rounded-none" is not allowed on <SaveButton>: <SaveButton> forwards className to <Button>, which owns its shape`,
      `designops/no-restyle: "p-4" is not allowed on <IconButton>: <IconButton> forwards className to <Button>, which owns its spacing`,
    ])
  })
})
