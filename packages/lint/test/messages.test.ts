...
// `settings.designops.note` trails every finding of every rule. Both fill the
...
const settings = { designops: { note: NOTE } }
...
describe("settings.designops.note", () => {
  ...
  test("a note that is not a string is ignored, with one warning", () => {
    ...
    expect(warnings.some((w) => w.includes("settings.designops.note"))).toBe(
      true
    )
  })
})
