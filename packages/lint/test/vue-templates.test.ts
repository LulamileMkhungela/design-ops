...
describe("Vue templates", () => {
  ...
  it("reads :class arrays, and an identifier from the script", () => {
    expect(
      ...
      `designops/no-restyle: "text-[#123456]" is not allowed on <CardContent>: <CardContent> owns its color`,
  ...
  it("reads a v-bind object as a spread", () => {
    expect(found(`<Card v-bind="{ class: 'rounded-none' }" />`)).toEqual([
      `designops/no-restyle: "rounded-none" is not allowed on <Card>: <Card> owns its shape`,
    ])
  ...
  it("reads style and :style", () => {
    expect(
      ...
      "designops/no-inline-styles: Inline style sets backgroundColor",
