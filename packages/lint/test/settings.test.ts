// settings.designops: recognition options written once for every rule, and
...
describe("settings.designops.ui", () => {
  ...
  test("a prefix makes every rule recognize the design system", () => {
    ...
    expect(found[0]).toMatchObject({ ruleId: "designops/no-restyle", line: 2 })
    ...
    ).toMatchObject([{ ruleId: "designops/require-static-classes", line: 2 }])
  })

  test("the prefix is literal, not a regex, and matches whole segments", () => {
    const other = `import { Button } from "@/dsx"\nexport const A = () => <Button className="bg-highlight">Go</Button>`
    expect(
      lint(NO_JSON, NO_JSON_PAGE, other, rules, { designops: { ui: "@/ds" } })
    ).toEqual([])
  })
  ...
  test("adds to a rule's own patterns instead of replacing them", () => {
    const found = lint(
      ...
      restyle,
      {
        "designops/no-restyle": ["error", { componentImports: ["^nothing$"] }],
      },
      { designops: { ui: "@/ds" } }
    )
    expect(found).toHaveLength(1)
  ...
  test("a wrong type warns once and is ignored", () => {
    expect(
      lint(NO_JSON, NO_JSON_PAGE, restyle, rules, { designops: { ui: 42 } })
    ).toEqual([])
    expect(warnings).toEqual([
      "[@designops/lint] settings.designops.ui must be a string or an array of strings; it is ignored.",
    ])
  })
})

describe("settings.designops as shared defaults", () => {
  test("componentImports applies to every rule", () => {
    const settings = { designops: { componentImports: ["^@/ds(/|$)"] } }
  ...
  test("mergeFunctions from settings counts for readability, as the option does", () => {
    ...
    const viaSetting = lint(PROJECT, PAGE, code, restyle, {
      designops: { mergeFunctions: ["merge"] },
    })
    expect(viaOption).toMatchObject([
      { ruleId: "designops/no-restyle", line: 3 },
    ])
  ...
  test("ignoreImports from settings hides a package", () => {
    ...
    expect(
      lint(PROJECT, PAGE, code, { "designops/no-restyle": "error" })
    ).toHaveLength(1)
    expect(
      lint(
        PROJECT,
        ...
        code,
        { "designops/no-restyle": "error" },
        { designops: { ignoreImports: ["^@/components/ui/"] } }
      )
    ).toEqual([])
  })
})

describe("settings.shadcn fallback", () => {
  test("a config written for the upstream plugin keeps working", () => {
    const found = lint(NO_JSON, NO_JSON_PAGE, restyle, rules, {
      shadcn: { ui: "@/ds" },
    })
    expect(found).toHaveLength(1)
  ...
  test("settings.designops wins when both namespaces are present", () => {
    expect(
      lint(NO_JSON, NO_JSON_PAGE, restyle, rules, {
        designops: { ui: "@/nothing" },
        shadcn: { ui: "@/ds" },
      })
    ).toEqual([])
  })

  test("the upstream note still trails every message", () => {
    const found = lint(NO_JSON, NO_JSON_PAGE, restyle, rules, {
      shadcn: { ui: "@/ds", note: "See DESIGN.md." },
    })
    expect(found).toHaveLength(1)
    expect(found[0].message).toContain("See DESIGN.md.")
  })
})
...
describe("allow validation", () => {
  ...
  test('"blue-500" names a color, not a class: a line-1 finding with the fix', () => {
    const found = lint(PROJECT, PAGE, red, {
      "designops/no-raw-colors": ["error", { allow: ["blue-500"] }],
    })
    expect(found).toHaveLength(1)
  ...
  test("a misspelled category is a line-1 finding with a suggestion", () => {
    const found = lint(
      ...
      {
        "designops/no-arbitrary-values": ["error", { allow: ["spacig"] }],
      }
    )
    expect(found).toHaveLength(1)
    expect(found[0]).toMatchObject({
    ...
    for (const allow of [
      ...
      expect(
        lint(PROJECT, PAGE, red, {
          "designops/no-raw-colors": ["error", { allow }],
        })
      ).toEqual([])
    }
    expect(
      lint(
        ...
        {
          "designops/no-arbitrary-values": ["error", { allow: ["w-[320px]"] }],
        }
      )
  ...
  test("a real class from the scale is not mistaken for a color", () => {
    const found = lint(PROJECT, PAGE, red, {
      "designops/no-raw-colors": ["error", { allow: ["opacity-50", "z-10"] }],
    })
    expect(found).toHaveLength(1)
  ...
  test("no-unknown-classes keeps accepting bare custom names", () => {
    expect(
      lint(
        ...
        {
          "designops/no-unknown-classes": ["error", { allow: ["toaster"] }],
        }
      )
    ).toEqual([])
  })
