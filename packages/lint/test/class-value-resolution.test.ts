...
describe("forwarded values retain their local alternatives", () => {
  test.each([
    ...
    expect(
      messages.filter((m) => m.ruleId === "designops/no-restyle")
    ).toHaveLength(2)
    expect(
      messages.filter((m) => m.ruleId === "designops/no-raw-colors")
    ).toHaveLength(1)
    expect(
      messages.some((m) => m.ruleId === "designops/require-static-classes")
    ).toBe(false)
  })
  ...
  test("a helper call escapes an object for later member reads", () => {
    ...
    ).toContain("designops/require-static-classes")
  })
