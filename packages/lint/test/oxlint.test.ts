...
// the file DESIGNOPS_LINT_PLUGIN points at.
...
const plugin = process.env.DESIGNOPS_LINT_PLUGIN
  ? path.resolve(process.env.DESIGNOPS_LINT_PLUGIN)
...
// source. `pnpm test` builds first; DESIGNOPS_LINT_PLUGIN names a side
...
const buildUsable = (() => {
  ...
  if (process.env.DESIGNOPS_LINT_PLUGIN) return true
  return (
...
function oxlint(
  ...
  const dir = fs.mkdtempSync(path.join(os.tmpdir(), "designops-lint-oxlint-"))
...
describe.skipIf(!buildUsable)("oxlint", { timeout: RUN_TIMEOUT }, () => {
  ...
  test("designops project: every rule reports, with variants, file paths, wrappers, SVG, unknown classes", () => {
    const out = oxlint(
    ...
    for (const rule of Object.keys(ALL_RULES)) {
      expect(out).toContain(`designops(${rule.slice("designops/".length)})`)
    }
  ...
  test("a contract message and the settings note reach the message", () => {
    const out = oxlint(
      ...
      { designops: { note: "See DESIGN.md." } }
    )
    // The contract's words replace the rule's text; the note trails.
    expect(out).toMatch(
      /designops\(no-restyle\): Buttons are variants only\. See DESIGN\.md\./
    )
  ...
  test("no components.json: settings.designops.ui, discovered theme, barrel variants", () => {
    const out = oxlint(
      path.join(FIXTURES, "no-json"),
      "src/app/oxlint.tsx",
      { "designops/no-restyle": "error", "designops/no-raw-colors": "error" },
      { designops: { ui: "@/ds" } }
    )
    expect(out).toMatch(
  ...
  test("an override turns a rule off for a directory, as the README's setup does", () => {
    const out = oxlint(
      ...
      [{ files: ["**/app/**"], rules: { "designops/no-restyle": "off" } }]
    )
    expect(out).not.toContain("designops(no-restyle)")
    expect(out).toContain("designops(no-raw-colors)")
  })
