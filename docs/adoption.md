...
```js
"designops/no-restyle": ["warn", { allow: ["layout"] }]
```
...
{
  files: ["app/**", "features/**"],
  rules: { "designops/no-restyle": ["error", { allow: ["layout"] }] },
},
{
  files: ["legacy/**"],
  rules: { "designops/no-restyle": ["warn", { allow: ["layout"] }] },
},
```
...
```js
{ files: ["marketing/**"], rules: { "designops/no-restyle": "off" } }
```
...
rules: {
  ...
  "designops/no-arbitrary-values": ["error", { allow: ["layout"] }],
...
```js
{
  ...
  rules: {
    ...
    "designops/require-static-classes": "off",
  },
}
```
...
```tsx
// eslint-disable-next-line designops/no-raw-colors -- Partner brand color, approved by design.
<span className="bg-amber-400">Sponsor</span>
```

`rg "eslint-disable.*designops/"` finds these comments. You can require a
