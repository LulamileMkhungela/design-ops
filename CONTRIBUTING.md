...
```bash
pnpm --filter @designops/lint exec tsdown --out-dir dist-next
DESIGNOPS_LINT_PLUGIN=packages/lint/dist-next/index.js node packages/evals/scripts/corpus.mjs
```

`dist-next/` is ignored by Git, ESLint, and Prettier. The corpus check
and Oxlint tests also accept `DESIGNOPS_LINT_PLUGIN`; relative paths are
...
1. Update `tailwindcss` in `packages/lint`, then regenerate the theme:
   ...
   ```bash
   pnpm --filter @designops/lint exec node scripts/generate-tailwind-theme.mjs
   ```
...
The temporary project is removed afterward. `DESIGNOPS_LINT_PLUGIN` selects
...
```bash
pnpm --filter @designops/lint exec node scripts/smoke-install.mjs
```
